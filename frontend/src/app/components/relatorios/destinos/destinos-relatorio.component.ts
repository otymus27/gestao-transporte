...
@Component({
  selector: 'app-destino-relatorio',
  imports: [CommonModule, FormsModule],
  templateUrl: './destinos-relatorio.component.html',
  styleUrl: './destinos-relatorio.component.scss',
})
export class DestinosRelatorioComponent {
  constructor(
    private destinoService: DestinoService,
    private relatorioService: RelatorioDestinoService
  ) {}
  ...
  exportar(tipo: ExportTipo): void {
    ...
    if (tipo === 'pdf') {
      this.relatorioService.exportarPdf(filtrosExport).subscribe({
        next: (blob) =>
          this.baixarArquivo(blob, 'rel_destinos.pdf', 'application/pdf'),
        error: (err) => {
  ...
  private exportarCsv(): void {
    ...
    this.baixarArquivo(blob, 'rel_destinos.csv', 'text/csv');
  }
